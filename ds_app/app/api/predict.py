...
df = pd.read_csv('https://raw.githubusercontent.com/bw-airbnb-2/DS/master/airbnb_bw2.csv', index_col=0)
dataset = df.values
X = dataset[:,0:12]
y = dataset[:,12]
y=np.reshape(y, (-1,1))
...
async def predict(item: Item):
    ...
    X_new = item.to_df()
    log.info(X_new)
    model = load_model("keras_nn_model.h5", compile=False)
    ...
    Xnew = np.array([[X_new['zipcode'].iloc[0], X_new['square_footage'].iloc[0], X_new['bedrooms'].iloc[0],
                      X_new['bathrooms'].iloc[0], X_new['review_score_rating'].iloc[0],
                      X_new['accommodates'].iloc[0], X_new['cleaning_fee'].iloc[0], float(free_park), 
